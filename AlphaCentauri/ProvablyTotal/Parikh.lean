...
existential quantifier bounded by a term. This is proven for $\forall\ldots\forall\exists`
formulas, which have any number of unbounded universal quantifiers, then one unbounded
existential.
- [HP98, Theorem V.1.4]
...
theorem parikh (φ : ArithmeticSemisentence (k + 1)) (hφ : φ.Bounded)
  ...
  -- consisting of `𝗜𝚺₀` plus, for each of the first `n` terms `t` of `ℒₒᵣ`, the sentence
  -- `∀(#0 < t), ∼φ`.
