...
The uniform reflection schema of an arithmetic theory, over formulas of arbitrary arity.
...
/-- The uniform reflection instance of `T` for a `k`-ary formula `φ`: the universal closure of
`Pr_T(⌜φ(ẋ₁, …, ẋ_k)⌝) → φ(x₁, …, x_k)`.
- [Lin97, §4.1, p. 52]
- [AB05, §4.2] -/
...
/-- The uniform reflection schema of `T` over a class `Γ` of formulas,
`RFN_Γ(T) = { ∀x⃗ (Pr_T(⌜φ(ẋ⃗)⌝) → φ(x⃗)) | Γ φ }`.
- [Lin97, §4.1, p. 52]
- [AB05, §4.2] -/
...
/-- Uniform reflection is monotone in the formula class.
- [Lin97, §4.1, p. 52]
- [AB05, §4.2] -/
