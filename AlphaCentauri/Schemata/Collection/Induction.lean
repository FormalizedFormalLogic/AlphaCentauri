...
lemma succ_induction_of_exists_pi
  ...
  have key : ∀ x, ∃ y < max v (w₀ + 1), Q x y ∨ a < x := by
    refine InductionOnHierarchy.succ_induction 𝚷 n (definable_bounded hQ a _)
      ⟨w₀, lt_of_lt_of_le (lt_add_one w₀) (le_max_right v (w₀ + 1)), Or.inl hw₀⟩ ?_
    rintro x ⟨y, -, hy | hy⟩
    · by_cases hxa : x < a
...
/-- Every model of `𝗕𝚺 (n + 1)` satisfies `𝗜𝚺 n`.
- [HP98, Lemma I.2.15] -/
...
/-- `𝗜𝚺 n` is at most as strong as `𝗕𝚺 (n + 1)`.
- [HP98, Lemma I.2.15] -/
