...
standard codes. -/
lemma mem_Δ₁Class_natCast_iff {m : ℕ} : m ∈ S.Δ₁Class ↔ (m : V) ∈ S.Δ₁Class := by
  simpa using Defined.shigmaOne_absolute V (φ := S.Δ₁ch)
    (R := fun v ↦ v 0 ∈ S.Δ₁Class) (R' := fun v ↦ v 0 ∈ S.Δ₁Class)
...
/-- A standard code of a formula that lies in the $\Delta_1$ class of `S` is the code of a member of
`S`. -/
lemma exists_mem_eq_quote {m : ℕ} (hmem : (m : V) ∈ S.Δ₁Class)
    (hsemi : IsSemiformula ℒₒᵣ (0 : V) (m : V)) : ∃ σ ∈ S, (m : V) = (⌜σ⌝ : V) := by
  obtain ⟨F, hF⟩ := IsSemiformula.sound (L := ℒₒᵣ) (isSemiformula_natCast_iff (V := V) |>.mpr hsemi)
  have : (⌜F⌝ : ℕ) ∈ S.Δ₁Class := by
...
lemma eval_collapseFormula_sigma (c : V) :
    V ⊧/![c] (collapseFormula T S n 𝚺) ↔
      ∃ y : V, (∃ u < y, Proof T u (neg ℒₒᵣ c)) ∧
        ∀ z < y, z ∈ S.Δ₁Class → IsSemiformula ℒₒᵣ (0 : V) z → IsStrictSigma (n + 1) z →
          SigmaSatisfaction (n + 1) z 0 := by
  simp [collapseFormula, HierarchySymbol.Semiformula.val_sigma,
    (Δ₁Class.defined (T := S) (V := V)).proper.iff',
    (Δ₁Class.defined (T := S) (V := V)).df,
...
lemma models_collapseSentence_pi_iff :
    V↓[ℒₒᵣ] ⊧ collapseSentence T S n 𝚷 ↔
      ∀ y ∈ S.Δ₁Class, IsSemiformula ℒₒᵣ (0 : V) y → IsStrictPi (n + 1) y →
        (∀ u < y, ¬Proof T u (⌜∼fixedpoint (collapseFormula T S n 𝚷)⌝ : V)) →
          PiSatisfaction (n + 1) y 0 := by
  have h : V↓[ℒₒᵣ] ⊧ collapseSentence T S n 𝚷 ↔
      V ⊧/![(⌜fixedpoint (collapseFormula T S n 𝚷)⌝ : V)] (collapseFormula T S n 𝚷) := by
...
lemma models_collapseSentence_sigma_iff :
    V↓[ℒₒᵣ] ⊧ collapseSentence T S n 𝚺 ↔
      ∃ y : V, (∃ u < y, Proof T u (⌜∼fixedpoint (collapseFormula T S n 𝚺)⌝ : V)) ∧
        ∀ z < y, z ∈ S.Δ₁Class → IsSemiformula ℒₒᵣ (0 : V) z → IsStrictSigma (n + 1) z →
          SigmaSatisfaction (n + 1) z 0 := by
  have h : V↓[ℒₒᵣ] ⊧ collapseSentence T S n 𝚺 ↔
      V ⊧/![(⌜fixedpoint (collapseFormula T S n 𝚺)⌝ : V)] (collapseFormula T S n 𝚺) := by
    simp [collapseSentence, models_iff]
...
private lemma neg_fixedpoint_of_neg_collapseSentence
    (h : T ⊢ ∼collapseSentence T S n Γ) : T ⊢ ∼fixedpoint (collapseFormula T S n Γ) :=
  neg_of_iff (WeakerThan.pbl (provable_fixedpoint_collapseFormula_iff T S n Γ)) h
...
private lemma neg_collapseSentence_of_neg_fixedpoint
    (h : T ⊢ ∼fixedpoint (collapseFormula T S n Γ)) : T ⊢ ∼collapseSentence T S n Γ :=
  neg_of_iff' (WeakerThan.pbl (provable_fixedpoint_collapseFormula_iff T S n Γ)) h
...
private lemma inconsistent_union_of_inconsistent_insert_pi
    (hΓ : ∀ σ ∈ S, StrictHierarchy 𝚷 (n + 1) σ)
    (h : Inconsistent (insert (collapseSentence T S n 𝚷) T)) : Inconsistent (T ∪ S) := by
  have : 𝗜𝚺₁ ⪯ T ∪ S := WeakerThan.trans (𝓣 := T) inferInstance inferInstance
  have : 𝗘𝗤 ℒₒᵣ ⪯ T ∪ S := WeakerThan.trans (𝓣 := 𝗜𝚺₁) inferInstance inferInstance
  have hneg : T ⊢ ∼collapseSentence T S n 𝚷 := neg_of_inconsistent_insert h
  have hnegζ : T ⊢ ∼fixedpoint (collapseFormula T S n 𝚷) :=
    neg_fixedpoint_of_neg_collapseSentence hneg
  have hprov : T ∪ S ⊢ collapseSentence T S n 𝚷 := by
    apply Arithmetic.complete.{0}
    intro M _ _
    have : M↓[ℒₒᵣ] ⊧* 𝗜𝚺₁ := ModelsTheory.of_provably_subtheory M 𝗜𝚺₁ (T ∪ S) inferInstance
    apply models_collapseSentence_pi_iff.mpr
    intro y hmem hsemi _ hlt
    have hp : Proof T ((⌜hnegζ.get⌝ : ℕ) : M) ⌜∼fixedpoint (collapseFormula T S n 𝚷)⌝ := by
...
private lemma inconsistent_union_of_inconsistent_insert_sigma
    (hΓ : ∀ σ ∈ S, StrictHierarchy 𝚺 (n + 1) σ)
    (h : Inconsistent (insert (collapseSentence T S n 𝚺) T)) : Inconsistent (T ∪ S) := by
  have : 𝗜𝚺₁ ⪯ T ∪ S := WeakerThan.trans (𝓣 := T) inferInstance inferInstance
  have : 𝗘𝗤 ℒₒᵣ ⪯ T ∪ S := WeakerThan.trans (𝓣 := 𝗜𝚺₁) inferInstance inferInstance
  have hneg : T ⊢ ∼collapseSentence T S n 𝚺 := neg_of_inconsistent_insert h
  have hnegζ : T ⊢ ∼fixedpoint (collapseFormula T S n 𝚺) :=
    neg_fixedpoint_of_neg_collapseSentence hneg
  have hprov : T ∪ S ⊢ collapseSentence T S n 𝚺 := by
    apply Arithmetic.complete.{0}
    intro M _ _
    have : M↓[ℒₒᵣ] ⊧* 𝗜𝚺₁ := ModelsTheory.of_provably_subtheory M 𝗜𝚺₁ (T ∪ S) inferInstance
    have hp : Proof T ((⌜hnegζ.get⌝ : ℕ) : M) ⌜∼fixedpoint (collapseFormula T S n 𝚺)⌝ := by
      simp [coe_quote_proof_eq]
...
/-- If adjoining `collapseSentence` to `T` is inconsistent then so is `T ∪ S`.
- [Lin97, Theorem 4.3] -/
private lemma inconsistent_union_of_inconsistent_insert
    (hΓ : ∀ σ ∈ S, StrictHierarchy Γ (n + 1) σ)
    (h : Inconsistent (insert (collapseSentence T S n Γ) T)) : Inconsistent (T ∪ S) := by
  cases Γ with
...
private lemma provable_of_mem_pi (hΓ : ∀ σ ∈ S, StrictHierarchy 𝚷 (n + 1) σ)
    (hcon : Consistent (insert (collapseSentence T S n 𝚷) T))
    {σ : ArithmeticSentence} (hσ : σ ∈ S) :
    insert (collapseSentence T S n 𝚷) T ⊢ σ := by
  have hnζ : ¬T ⊢ ∼fixedpoint (collapseFormula T S n 𝚷) := fun h ↦
    hcon.not_inc (inconsistent_insert_of_provable_neg (neg_collapseSentence_of_neg_fixedpoint h))
  have key : 𝗜𝚺₁ ⊢ collapseSentence T S n 𝚷 🡒 σ := by
    apply Arithmetic.complete.{0}
    intro M _ _
  ...
  have hT : T ⪯ insert (collapseSentence T S n 𝚷) T := WeakerThan.ofSubset (Set.subset_insert _ _)
  have key' : insert (collapseSentence T S n 𝚷) T ⊢ collapseSentence T S n 𝚷 🡒 σ :=
    hT.pbl (WeakerThan.pbl (𝓢 := 𝗜𝚺₁) key)
  exact key' ⨀ by_axm (Set.mem_insert _ _)

/-- The polarity `𝚺` case of `provable_of_mem`. -/
private lemma provable_of_mem_sigma (hΓ : ∀ σ ∈ S, StrictHierarchy 𝚺 (n + 1) σ)
    (hcon : Consistent (insert (collapseSentence T S n 𝚺) T))
    {σ : ArithmeticSentence} (hσ : σ ∈ S) :
    insert (collapseSentence T S n 𝚺) T ⊢ σ := by
  have hnζ : ¬T ⊢ ∼fixedpoint (collapseFormula T S n 𝚺) := fun h ↦
    hcon.not_inc (inconsistent_insert_of_provable_neg (neg_collapseSentence_of_neg_fixedpoint h))
  have key : 𝗜𝚺₁ ⊢ collapseSentence T S n 𝚺 🡒 σ := by
    apply Arithmetic.complete.{0}
    intro M _ _
    simp only [Semantics.Imp.models_imply]
    intro hθ
  ...
  have hT : T ⪯ insert (collapseSentence T S n 𝚺) T := WeakerThan.ofSubset (Set.subset_insert _ _)
  have key' : insert (collapseSentence T S n 𝚺) T ⊢ collapseSentence T S n 𝚺 🡒 σ :=
    hT.pbl (WeakerThan.pbl (𝓢 := 𝗜𝚺₁) key)
  exact key' ⨀ by_axm (Set.mem_insert _ _)

/-- A consistent extension of `T` by `collapseSentence` proves every member of `S`.
- [Lin97, Theorem 4.3] -/
private lemma provable_of_mem (hΓ : ∀ σ ∈ S, StrictHierarchy Γ (n + 1) σ)
    (hcon : Consistent (insert (collapseSentence T S n Γ) T))
    {σ : ArithmeticSentence} (hσ : σ ∈ S) :
    insert (collapseSentence T S n Γ) T ⊢ σ := by
  cases Γ with
...
/-- The case of an extension by `S` itself, all of whose members are
`StrictHierarchy Γ (n + 1)`.
- [Lin97, Theorem 4.3] -/
private lemma exists_sentence_weakerThan_of_forall_mem
    (hΓ : ∀ σ ∈ S, StrictHierarchy Γ (n + 1) σ) [Consistent (T ∪ S)] :
    ∃ θ : ArithmeticSentence, Hierarchy Γ (n + 1) θ ∧
      T ∪ S ⪯ insert θ T ∧ Consistent (insert θ T) := by
  have hcon : Consistent (insert (collapseSentence T S n Γ) T) := by
    by_contra hc
    exact (inconsistent_union_of_inconsistent_insert hΓ
      (not_consistent_iff_inconsistent.mp hc)).not_con inferInstance
  refine ⟨collapseSentence T S n Γ, hierarchy_collapseSentence T S n Γ, ?_, hcon⟩
  apply WeakerThan.ofAxm!
  rintro φ (hφ | hφ)
  · exact by_axm (Set.mem_insert_of_mem _ hφ)
  · exact provable_of_mem hΓ hcon hφ

/-- If `T ∪ U` is consistent, for a theory `U` axiomatized by a $\Delta_1$-presented set `S` of
`StrictHierarchy Γ (n + 1)` sentences, then there is a `Γ (n + 1)` sentence `θ` such that
`T ∪ U ⪯ insert θ T` and `insert θ T` is consistent.

The axiomatization `S` is read as $\Delta_1$-presented rather than r.e.: this is a deliberate
narrowing of [AB05]'s "consistent r.e. extension", which reduces to an elementary presentation via
Craig's trick, a result absent from both Foundation and this repository. Restricting `S`'s members
...
theorem exists_sentence_weakerThan_of_consistent
    (hΓ : AxiomatizableBy (StrictHierarchy Γ (n + 1)) U S) [Consistent (T ∪ U)] :
    ∃ θ : ArithmeticSentence, Hierarchy Γ (n + 1) θ ∧
      T ∪ U ⪯ insert θ T ∧ Consistent (insert θ T) := by
  have e : T ∪ U ≊ T ∪ S := hΓ.equiv.union_right T
  have : Consistent (T ∪ S) := Consistent.of_le ‹Consistent (T ∪ U)› e.symm.le
...
axiomatized by a $\Delta_1$-presented set `S` of `StrictHierarchy Γ (n + 1)` sentences, proves the
local reflection schema of `T` on the dual class, then `T ∪ U` is inconsistent.

As in `exists_sentence_weakerThan_of_consistent`, `S` is read as $\Delta_1$-presented rather than
...
theorem inconsistent_of_localReflectionOnHierarchy_weakerThan_union
    (hΓ : AxiomatizableBy (StrictHierarchy Γ (n + 1)) U S)
...
theory `U` axiomatized by a $\Delta_1$-presented set `S` of `StrictHierarchy Γ (n + 1)` sentences,
