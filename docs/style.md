...
`primrec` (`AlphaCentauri/Tactic/Primrec.lean`) proves `Primrec f`, `Primrec₂ f`, `PrimrecPred p` and `PrimrecRel r` by reading the shape of the function off the goal, so a composition of Mathlib's point-free combinators no longer has to be written by hand. Use it in place of `Primrec.comp`/`Primrec.fst`/`Primrec.snd` chains; `primrec?` prints what it found.
