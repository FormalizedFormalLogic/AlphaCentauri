...
lemma interpret_seq (B : FFL.ProvabilityLogic.Formula α) (i : ℕ) :
  ...
  | succ i ih =>
    ...
    simp only [seq, collapseSeq, FFL.ProvabilityLogic.Formula.interpret]
    cl_prover [ih, hb]
...
lemma collapseSeq_bot (i : ℕ) : T ⊢ 𝔅.collapseSeq ⊥ i 🡘 𝔅^[i] ⊥ := by
  ...
  | succ i ih =>
    ...
    have hm : T ⊢ 𝔅^[i] ⊥ 🡒 𝔅^[i + 1] ⊥ := WeakerThan.pbl (𝔅.iterate_bot_imp_iterate_bot i 1)
...
theorem iterate_bot_of_refutable_localReflection [𝔅.HBL] [Diagonalization T₀] {m : ℕ}
  ...
  have hr : T ⊢ (∼𝔅^[m + 1] ⊥ : Sentence L) 🡒
    ...
    right_Uconj_intro _ _ fun i ↦ by
      ...
      have hk : (i : ℕ) + 1 + (m - i) = m + 1 := by have := i.isLt; omega
      have hm : T ⊢ 𝔅^[i + 1] ⊥ 🡒 𝔅^[m + 1] ⊥ := by
        have h' : T ⊢ 𝔅^[i + 1] ⊥ 🡒 𝔅^[i + 1 + (m - i)] ⊥ :=
          WeakerThan.pbl (𝔅.iterate_bot_imp_iterate_bot (i + 1) (m - i))
        rwa [hk] at h'
      cl_prover [e, eb, hm]
  cl_prover [hc, hr]
