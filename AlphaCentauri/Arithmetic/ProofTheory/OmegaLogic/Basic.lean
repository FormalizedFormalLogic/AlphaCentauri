...
Neither [HP98] nor [Lin97] treats ω-logic, ordinal heights, or ordinal analysis; there is
therefore no citation from those two texts for anything in this file. The presentation followed is
Towsner's expository notes, cited as `[Tow20]`:

> Henry Towsner, *Goodstein's theorem, `ε₀`, and unprovability*, expository notes, University of
> Pennsylvania, 2020, <https://www.sas.upenn.edu/~htowsner/GoodsteinsTheorem.pdf>.
-/
...
/-- **Bounded derivability** `Z_∞ ⊢^α_c Γ`: some derivation of `Γ` has ordinal height at most `α`
and all its cut formulas have quantifier rank below `c`. Towsner's numeric side bound is dropped.
...
lemma contr (h : Provable α c (insert φ (insert φ Γ))) : Provable α c (insert φ Γ) := by
  simpa using h
...
lemma axTrue (b : Bool) (r : (ℒₒᵣ).Rel k) (v) (ht : LitTrue (signedLit b r v))
    (hmem : signedLit b r v ∈ Γ) : Provable 0 0 Γ :=
...
- [Tow20, Section 13] -/
lemma exI (n : ℕ) (h : Provable α c (insert (φₓ/[(↑n : ArithmeticTerm ℕ)]) Γ)) :
...
lemma allω {βₓ : ℕ → Ordinal.{0}}
    (h : ∀ n, Provable (βₓ n) c (insert (φₓ/[(↑n : ArithmeticTerm ℕ)]) Γ)) :
...
- [Tow20, Section 13] -/
lemma cut (χ : ArithmeticFormula ℕ) (hc : χ.qr < c) (h₁ : Provable α c (insert χ Γ))
    (h₂ : Provable β c (insert (∼χ) Γ)) : Provable (max α β + 1) c Γ := by
  obtain ⟨D₁, ho₁, hcr₁⟩ := h₁
  obtain ⟨D₂, ho₂, hcr₂⟩ := h₂
...
- [Tow20, Section 14] -/
lemma sound (h : Provable α c Γ) : ∃ φ ∈ Γ, LitTrue φ := h.choose.sound

/-- The empty sequent is derivable at no height and no cut rank. -/
lemma not_empty : ¬Provable α c ∅ := fun h => by simpa using h.sound
...
private lemma em_binaryStep {A B C D : ArithmeticFormula ℕ} (hab : A ⋏ B ∈ Γ) (hcd : C ⋎ D ∈ Γ)
    (h₁ : Provable α 0 (insert A (insert C (insert D Γ))))
    (h₂ : Provable α 0 (insert B (insert C (insert D Γ)))) : Provable (α + 1 + 1) 0 Γ := by
...
private lemma em_quantStep {φₓ ψₓ : ArithmeticSemiformula ℕ 1} (hall : (∀¹ φₓ) ∈ Γ)
    (hexs : (∃¹ ψₓ) ∈ Γ)
    (fam : ∀ n : ℕ, Provable α 0
      (insert (ψₓ/[(↑n : ArithmeticTerm ℕ)]) (insert (φₓ/[(↑n : ArithmeticTerm ℕ)]) Γ))) :
    Provable (α + 1 + 1) 0 Γ := by
  have h : ∀ n : ℕ, Provable (α + 1) 0 (insert (φₓ/[(↑n : ArithmeticTerm ℕ)]) Γ) :=
...
private lemma lemAux (hk : φ.complexity ≤ k) (hp : φ ∈ Γ) (hn : ∼φ ∈ Γ) :
    Provable ((2 * k : ℕ) : Ordinal.{0}) 0 Γ := by
...
theorem lem (hp : φ ∈ Γ) (hn : ∼φ ∈ Γ) :
    Provable ((2 * φ.complexity : ℕ) : Ordinal.{0}) 0 Γ := lemAux le_rfl hp hn
  ...
  | succ k ih =>
    ...
    | hor φ ψ =>
      ...
      · have h₂ := ih (φ := ψ) (Γ := insert φ (insert ψ Γ)) (by omega) h (by simp)
        exact h₂.orI.insert_absorb hmem
    | hall ψ =>
      have h : ∀ n : ℕ, Provable (k : Ordinal.{0}) 0
...
theorem of_true (ht : LitTrue φ) (hmem : φ ∈ Γ) :
    Provable (φ.complexity : Ordinal.{0}) 0 Γ := of_trueAux le_rfl ht hmem
