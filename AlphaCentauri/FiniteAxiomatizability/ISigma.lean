...
Both statements are recorded here as axioms; the proof needs the partial truth predicates
`Sat_{Σ,n}`, not yet formalized, and the API around `Entailment.FiniteAxiomatizable`, not yet
grown, so it is deferred to a later revision of this file. `𝗕𝚺`, the collection
