...
composition is mechanical, so this module hands it to Aesop.
...
`reducible` the same failure is immediate. On the goals of `AlphaCentauri.ToMathlib.ONote` this
is the difference between a minute and a few milliseconds.
...
attribute [aesop 1 (rule_sets := [Primrec]) safe apply (transparency := reducible)]
  ...
  Primrec.list_length Primrec.list_range Primrec.vector_toList Primrec.vector_ofFn'
...
variable {α : Type*} [Primcodable α] {k : ℕ} {F : α → ℕ → ℕ} {f g : List ℕ → ℕ → ℕ}
